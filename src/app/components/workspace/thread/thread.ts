 ...
 *
 * Reaction-Logik ist bewusst eine eigene, generalisierte Kopie und NICHT
 * MainChatReactionService: die dortige Version ist fest an Channel-
 * Nachrichten gekoppelt (toggleReaction(message, emoji, channelId) ruft
 * intern immer addChannelReaction/removeChannelReaction). Thread-Antworten
 * koennen aber aus Channels ODER Direktchats stammen (parentMessage.dmId),
 * dafuer gibt es addReplyReaction/removeReplyReaction im MessageService,
 * die ueber die Elternnachricht selbst herausfinden, wohin geschrieben
 * wird - ohne MainChatReactionService umzubauen (Michaels aktive Datei).
 * Die Werte (Emoji-Liste, Pillen-Stil, Overflow-Verhalten) sind exakt
 * gleich wie im main-chat.
 */
@Component({
  selector: 'app-thread',
...
export class Thread implements OnDestroy {
  ...
  // keine Update-Funktion, und das betrifft potenziell auch Michaels
  // Main-Chat (gleiche Nachrichten-Struktur). Erst mit ihm abstimmen, dann
  // gemeinsam eine editMessage()-Methode + Bearbeiten-Modus bauen.
